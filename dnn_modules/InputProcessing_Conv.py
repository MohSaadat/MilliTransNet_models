...
class _ReduceShapeCode(nn.Module):
    def __init__(self,
                 ...
                 az_size,
                 el_size,
                 seqL, # sequence_length will act as groups
                 shape_code_size,
                 hidden_conv_channels=[2**2,2**4],
                 #hidden_lin_dims=[256],
                 lRelu_slope=0.05,
                 dout_per=0.3):
        super(_ReduceShapeCode,self).__init__()

        self.__rd_size = rd_size
        self.__az_size = az_size
        self.__el_size = el_size
        self.__seqL = seqL
        self.__shape_code_size = shape_code_size
        self.__hidden_conv_channels = hidden_conv_channels
        #self.__hidden_lin_dims = hidden_lin_dims
        self.__lRelu_slope = lRelu_slope
        self.__dout_per = dout_per
        self.__build_network()

    def __build_network(self):
        self.__conv1 = Conv2DBlock(inChannel=1*self.__seqL*self.__rd_size,
                                    outChannel=self.__hidden_conv_channels[0]*self.__seqL*self.__rd_size,
                                    groups=self.__seqL*self.__rd_size,
                                    bias=True,
                                    kernel=(4,4),
                                    stride=(4,4),
                                    padding=(0,0),
                                    ...
                                    dout=True,
                                    dout_per=self.__dout_per)
        self.__conv2 = Conv2DBlock(inChannel=self.__hidden_conv_channels[0]*self.__seqL*self.__rd_size,
                                    outChannel=self.__hidden_conv_channels[1]*self.__seqL*self.__rd_size,
                                    groups=self.__seqL*self.__rd_size,
                                    bias=True,
                                    kernel=(4,4),
                                    stride=(4,4),
        ...
        sp1, sp2 = self.__conv1.get_out_size([self.__az_size,self.__el_size])
        self.__sp1, self.__sp2 = self.__conv2.get_out_size([sp1, sp2])

        self.__linear = LinearBlock(
            inChannel=self.__rd_size*self.__hidden_conv_channels[1]*self.__sp1*self.__sp2,
            outChannel=self.__shape_code_size,
            dout=True,
            lRelu_slope=self.__lRelu_slope,
            dout_per=self.__dout_per)

        #self.__linear1 = LinearBlock(inChannel=self.__rd_size*self.__hidden_conv_channels[2]*self.__sp1*self.__sp2,
        #                              outChannel=self.__hidden_lin_dims[0],
        #                              dout=True,
        #                              lRelu_slope=self.__lRelu_slope,
        #                              dout_per=self.__dout_per)
        #self.__linear2 = LinearBlock(inChannel=self.__hidden_lin_dims[0],
        #                              outChannel=self.__shape_code_size,
        #                              dout=True,
        #                              lRelu_slope=self.__lRelu_slope,
        #                              dout_per=self.__dout_per)
    ...
    def forward(self,input_signal):
        '''
        :param input_signal: b x seqL*rd_size x az_size x el_size
        :return: b x seqL x shape_code_size
        '''
        b = input_signal.size(0)
        x = self.__conv1(input_signal)
        x = self.__conv2(x)

        x = x.view(b,
                   self.__seqL,
                   self.__rd_size,
                   self.__hidden_conv_channels[1],
                   self.__sp1,
                   self.__sp2)
        x = self.__flatten1(x)
...
class CreateShapeCode(nn.Module):
    ...
    def __build_network(self):
        ...
        self.__rd_processing = _RD_processing(self.__nRanges,self.__nDoppler,groups)
        self.__cross_channel = _CrossChannel(self.__short_channel_size,self.__long_channel_size)
        self.__reduce_shape_code = _ReduceShapeCode(self.__rd_processing.get_outchannel(),
                                                    self.__cross_channel.get_outchannel(),
                                                    self.__cross_channel.get_outchannel(),
                                                    self.__seqL,
    ...
    def __prepare_for_reduce_shape_code(self,input):
        ...
        input = torch.reshape(input.transpose(1,2),
                              (-1,
                               self.__seqL * self.__rd_processing.get_outchannel(),
                               self.__cross_channel.get_outchannel(),
                               self.__cross_channel.get_outchannel()
                               )
                              ) # b x seqL*rd_out x h_size x v_size
        return input
...
if __name__ == "__main__":
    b, nR, nD, short_size, long_size, seqL = 4, 104, 32, 2, 16, 32
